...
void StochasticProcess::initialize_generator(
    const std::function<double(double)> &process_fn,
    const std::function<double(std::mt19937 &)> &dist_fn) {

  // assign lambda to distribution function
  distribution_function = dist_fn;
  process = process_fn;
  // default behavior, simulates white noise with
